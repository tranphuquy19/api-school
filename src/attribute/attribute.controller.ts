...
export class AttributeController {
    constructor(private readonly nameService: AttributeService) { }
    ...
    @Post()
    @ApiResponse({ type: CreateAttributeDto, status: 200 })
    ...
    @UseGuards(new JwtAuthGuard()) // Protección de rutas para el endpoint
    @ApiHeader({ name: 'Authorization', required: true })
    @ApiResponse({ type: [CreateAttributeDto], status: 201 })
    @Get()
    async list() {
    ...
    @ApiResponse({ type: CreateAttributeDto, status: 201 })
    @ApiHeader({ name: 'Authorization', required: true })
    async update(@Param('id') id: number, @Body() body: Partial<UpdateAttributeDto>) {
    ...
    @UseGuards(new JwtAuthGuard()) // Protección de rutas para el endpoint
    @ApiHeader({ name: 'Authorization', required: true })
    @ApiResponse({ status: 201, description: 'attribute_deleted'})
